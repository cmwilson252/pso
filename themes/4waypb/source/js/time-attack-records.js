...
window.fourwaypb.time_attack_records.ready = function() {
    ...
    function updateGroups(table) {
        ...
        if (group_settings.meta) {
            groups.push('meta');
        }
        if (group_settings.episode) {
            groups.push('episode');
        }
        if (group_settings.quest) {
            groups.push('quest.name');
        }
        if (group_settings.pb) {
            groups.push('pb');
        }
        
        results_table.setGroupBy(groups);
    }
            ...
            columns: [
                {
                    ...
                    titleFormatter: function() {
                        ...
                        let group_icon = document.createElement('i');
                        
                        title.innerText = 'Meta';
                        
                        group_icon.classList.add('fas', 'fa-object-group');
                        group_icon.addEventListener('click', function(e) {
                            e.preventDefault();
                            e.stopPropagation();
                            
                            group_settings.meta = !group_settings.meta;
                            
                            group_icon.classList.remove('fa-object-group', 'fa-object-ungroup');
                            if (group_settings.meta) {
                                group_icon.classList.add('fa-object-ungroup');
                            } else {
                                group_icon.classList.add('fa-object-group');
                            }
                            
                            updateGroups();
                        })
                        group_button.append(group_icon);
                        
                        result.classList.add('header-container');
                        result.append(title);
                        result.append(group_button);
